...
Public Class cancelSaleForm
    ...
    Private Sub modifySalesForm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        conn.ConnectionString = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=D:\AccountSW\Data\" & comId & "\" & comId & ".mdb"
        If conn.State = ConnectionState.Closed Then
            conn.Open()
        End If
        Dim sql As String = "select Vch_No from Sales Group by Vch_No"
        ...
        While i < t
            If selectVchNoCb.Text = selectVchNoCb.Items.Item(i) Then
                If MessageBox.Show("Do you really want to Cancel this Voucher?", "Delete", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = DialogResult.Yes Then
                    Dim sql As String = "select Product, Qty from Sales where Vch_No = '" & selectVchNoCb.Text & "'"
                    da = New OleDbDataAdapter(sql, conn)
                    ...
                    For j = 0 To (dtTemp.Rows.Count - 1)
                        ...
                        da = New OleDbDataAdapter(sql, conn)
                        da.Fill(dtTemp2)
                        sql = "update ItemStock set Item_Qty = " & (dtTemp2.Rows(0).Item(0) + CInt(dtTemp.Rows(j).Item(1))) & " where Item_Name = '" & dtTemp.Rows(j).Item(0) & "'"
                        cmd = New OleDbCommand(sql, conn)
                        cmd.ExecuteNonQuery()
                        dtTemp2.Clear()
                    Next
                    sql = "select sum(Amount), Acc_Id from Sales where Vch_No = '" & selectVchNoCb.Text & "' Group by Vch_No, Acc_Id"
                    da = New OleDbDataAdapter(sql, conn)
                    Dim dtTemp3 As New DataTable
                    da.Fill(dtTemp3)
                    sql = "select Acc_Bal from Accounts where Acc_Id = '" & dtTemp3.Rows(0).Item(1) & "'"
                    da = New OleDbDataAdapter(sql, conn)
                    Dim dtTemp4 As New DataTable
                    da.Fill(dtTemp4)
                    sql = "update Accounts set Acc_Bal = " & dtTemp4.Rows(0).Item(0) - dtTemp3.Rows(0).Item(0) & " where Acc_Id = '" & dtTemp3.Rows(0).Item(1) & "'"
                    cmd = New OleDbCommand(sql, conn)
                    t = cmd.ExecuteNonQuery()
                    sql = "delete from Sales where Vch_No = '" & selectVchNoCb.Text & "'"
                    cmd = New OleDbCommand(sql, conn)
                    t = cmd.ExecuteNonQuery()
                    MsgBox("Sale Voucher Canceled!")
                    selectVchNoCb.Text = ""
                    dt.Clear()
                End If
                Exit While
    ...
    Private Sub cancelSaleForm_Move(sender As Object, e As EventArgs) Handles Me.Move
